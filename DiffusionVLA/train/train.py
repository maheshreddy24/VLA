...
def train_loop(cfg: FrankaConfig) -> None:
    ...
    #! this is the observation encoder, creates the encoder text, image and qpos
    # image encoder: resnet50
    # clip_text
    # obs_encoder: combines all of them

    #! mahesh read this
    """
        the obs_encoder does everything, it has the isntance of text encoder, image encoder,
        the input is a dict of 'qpos', 'image', 'text', it fuses the mebeddings of text and the qpos with a linear projection layers, 
        and then it gets the image embeddings from the image encoder, and projects them to n_emb dimension
        finally it returns a tensor of shape (B, 2, n_emb) where 2 is the number of tokens per timestep: [qpos+text, image]

        theres a small learnable projection layers for all whci will outptu same dim
        the qpos: (b, 7) + text (n_emb) --proj--> (n_emb)
        the image: resnet50 --proj--> (n_emb)
    """
    ...
    """
        what is the input for diffusion transformer? 
    ...
    import tqdm
    for epoch in tqdm.tqdm(range(start_epoch, cfg.num_epochs), desc="Epoch", total=cfg.num_epochs):
        model.train()
        obs_encoder.train()
        image_encoder.train()
        epoch_losses = []
